...
export default function Nav({showNav}) {
   ...
   async function logout(){
    ...
    return (
        <aside className={(showNav?'left-0':'-left-full')+
        ' top-0 text-gray-500 p-4 fixed w-full bg-bgGray h-full md:static md:w-auto transition-all'}>
            ...
            <nav className={'flex flex-col gap-2'}>
                ...
                <Link href={'/settings'} className={pathName.includes('/setting') ? activeLink : inactiveLink}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className={pathName === '/setting' ? activeIcon : inactiveIcon}>
